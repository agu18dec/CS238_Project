...
# I TRIED TO GET THIS FILE WORKING BUT IT WAS SUUUPER OUTDATED. 
# NOW IT WORKS BUT IS STILL WEIRD
...
if total_steps  < 20:
    ...
    action[1] = 0    # 1-chlorohexane
    action[2] = 0    # 2-chlorohexane
    action[3] = 1    # 3-chlorohexane
    action[4] = 1    # Na
while not done:
    # Actions:
    ...
    if ("1-chlorohexane" in env.unwrapped.shelf[0].material_dict.keys()):
        reactant_1.append(env.unwrapped.shelf[0].get_material_dataframe()['Amount'][0])
    else:
        reactant_1.append(0)
    # reactant_1.append(env.unwrapped.shelf[0].get_material_dataframe()['Amount'][2])
    reactant_2.append(env.unwrapped.shelf[0].material_dict['Na'].mol)
...
if show_stats.lower() in ["y", "yes"]:
    ...
    print("")
    print("---------- MATERIAL_DICT ----------")
    for material, value_list in v.material_dict.items():
        print("{} : {}".format(material, value_list))

    # print("")
    # print("---------- SOLUTE_DICT ----------")
    # for solute, value_list in v.solute_dict.items():
